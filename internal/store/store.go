...
// Zilvinas Vilutis: pulled from https://github.com/otoolep/hraftd/blob/master/store/store.go
// modified to use
...
func (s *Store) Open(enableSingle bool, localID string) error {
	...
	if s.inmem {
		...
		boltDB, err := raftboltdb.New(raftboltdb.Options{
			Path: filepath.Join(s.RaftDir, "store.db"),
		})
		if err != nil {
...
func (s *Store) Join(nodeID, addr string) error {
	...
	if err := configFuture.Error(); err != nil {
		s.logger.Printf("failed to get store configuration: %v", err)
		return err
	}
